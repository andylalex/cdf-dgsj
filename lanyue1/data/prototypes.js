/* 内容唯一权威源 —— 由管理模式经 serve.py 回写；改内容请用管理模式，勿手工改结构 */
window.PROTO_DATA = {
 "v": 3,
 ...
 "items": {
  ...
  "p_mtobsr6sh8yw": {
   ...
   "api": [],
   "acceptance": [
    "顾问入口可聚合展示品牌馆页、品牌主页、商品详情页、搜品牌页、我的页共五个二级入口",
    "点击任一入口可在手机壳内切换并加载对应真实页面",
    "导购端与用户端均可访问顾问入口",
    "页面切换后返回入口不丢失当前上下文",
    "任一二级入口加载失败时有兜底提示且不阻塞其他入口"
   ],
   "note": "",
   "hotspots": []
  },
  ...
  "v04-u1": {
   ...
   "images": [],
   "overview": {
    "summary": "用户在顾问 IM 会话中接收导购发送的商品/品牌/优惠券卡片，可查看、领券并进入商品详情；会话承接奢品顾问列表与消息列表的流转。",
    "path": "用户端 → 顾问消息列表 → 顾问 IM 会话页（U1）",
    "users": "C端用户",
    "permission": "功能权限：所有商城用户均有；数据权限：仅本人会话",
    "ports": "中免海南 APP（安卓、IOS、鸿蒙）、中免海南微信小程序、中免海南支付宝小程序、H5"
   },
   "flow": [
    {
   ...
   "api": [],
   "acceptance": [
    "会话展示导购发送的文字/商品推荐卡片/品牌卡片/优惠券卡片",
    "点击商品卡片进入 U3 商品详情并进入导购模式（业绩归因）",
    "点击优惠券卡片可领取优惠券，领取后按钮变为已领取",
    "点赞顾问等互动功能可用",
    "顶部展示顾问信息（门店/专柜）与企业微信名片入口",
    "会话仅本人可见，不串号"
   ],
   "note": "",
   "hotspots": [
  ...
  "v04-u6": {
   ...
   "overview": {
    ...
    "permission": "功能权限：导购可见；数据权限：仅当前会话顾客的商品与清单",
  ...
  "v04-u5": {
   ...
   "images": [],
   "overview": {
    "summary": "用户在结算前填写离岛行程（离岛方式、日期、航班/车次/船次、证件信息）以核验免税购买资格，支持离岛至境内/境外切换，核验通过方可选择提货方式。",
    "path": "用户端 → 结算中心（U4）→ 行程核验（U5）→ 返回结算",
    "users": "C端用户（离岛免税购物旅客）",
    "permission": "功能权限：结算用户可见；数据权限：仅本人行程与证件信息",
    "ports": "中免海南 APP（安卓、IOS、鸿蒙）、中免海南微信小程序、中免海南支付宝小程序、H5"
   },
   "flow": [
    {
   ...
   "api": [],
   "acceptance": [
    "支持离岛至境内 / 离岛至境外 Tab 切换",
    "填写姓名、国家/地区、证件类型、证件号、手机号",
    "选择离岛方式（飞机/火车/轮渡）并填写离岛日期与航班/车次/船次",
    "已离开海南时展示会员优选引导",
    "提交后系统校验行程与免税资格（规则 R-U5-01）",
    "校验不通过给出具体原因并可修改",
    "校验通过返回结算中心并可选提货方式"
   ],
   "note": "",
  ...
  "v04-u4": {
   ...
   "overview": {
    ...
    "users": "C 端用户",
    "permission": "功能权限：商城用户均有；数据权限：仅本人订单",
   ...
   "api": [],
   "acceptance": [
    "展示提货方式（离岛自提/邮寄到家/返岛自提）三 Tab",
    "展示商品清单、发票/优惠券/政府消费券/会员积分入口与费用明细",
    "提供行程核验入口跳转 U5",
    "需勾选同意条款方可提交",
    "仅展示本人订单商品",
    "提交后跳转支付成功页（U17）"
  ...
  "v04-u16": {
   ...
   "overview": {
    ...
    "path": "用户端 → 我的 → 订单列表 → 订单详情（U16）",
    "users": "C端用户",
    "permission": "功能权限：下单用户可见；数据权限：仅本人订单",
    "ports": "中免海南 APP（安卓、IOS、鸿蒙）、中免海南微信小程序、中免海南支付宝小程序、H5"
   },
   "flow": [],
   "states": [],
   ...
   "acceptance": [
    "顶部状态卡展示订单状态（含已取消态）",
    "展示取货人信息（姓名/手机号/证件/离岛信息/自提地址）",
    "订单编号支持复制",
    "展示离岛自提商品列表（商品名/规格/实付/专柜导购）",
    "提供申请发票、使用优惠券、退货积分补款入口",
    "底部提供返回首页入口"
   ],
   "note": "",
   "hotspots": []
  ...
  "v04-g1": {
   ...
   "overview": {
    ...
    "path": "中免商户通 → 工作台",
    "users": "门店导购",
    "permission": "功能权限：导购可见；数据权限：不涉及数据权限",
    "ports": "中免商户通APP"
   },
   "flow": [
    {
   ...
   "api": [],
   "acceptance": [
    "工作台展示店招与经营数据四宫格（访客/支付订单/支付金额/客单价及环比）",
    "展示数据趋势折线图",
    "常用功能含商品管理、订单、售后、业绩管理、顾问资料入口",
    "点击业绩管理跳转 G8",
    "点击顾问资料跳转 G9"
   ],
   "note": "",
  ...
  "v04-g2": {
   ...
   "overview": {
    ...
    "users": "门店导购",
    "permission": "功能权限：所有会话页功能权限；数据权限：不涉及数据权限",
    "ports": "中免商户通APP"
   },
   "flow": [
    {
  ...
  "v04-g3": {
   ...
   "images": [],
   "overview": {
    "summary": "导购在 IM 会话中与顾客实时沟通，可发送文字、商品推荐卡片、企业微信名片、图片，并通过 + 面板推荐商品、发优惠券，促进下单。",
    "path": "中免商户通 → 消息列表 → 消息会话页（G3）",
    "users": "门店导购",
    "permission": "功能权限：所有会话页；数据权限：仅本人会话顾客",
    "ports": "中免商户通APP"
   },
   "flow": [
    {
  ...
  "v04-g4": {
   ...
   "images": [],
   "overview": {
    "summary": "导购在 IM 会话中通过 + 面板进入商品选择器，按柜组/分类/品牌检索商品，查看门店与电商仓库存，发送商品推荐卡片或加入推荐。",
    "path": "中免商户通 → 消息会话（G3）→ +号 → 推荐商品 → 选择商品（G4）",
    "users": "门店导购",
    "permission": "功能权限：导购可见；数据权限：当前门店可售商品与库存",
    "ports": "中免商户通APP"
   },
   "flow": [
    {
   ...
   "acceptance": [
    "顶部搜索支持商品名称 / 编号 / 品牌检索",
    "左侧分类含我的收藏/我的柜组/其他柜组，可展开二级分类",
    "二级品牌分类（迪奥/兰蔻/香奈儿/浪凡等）横向筛选",
    "商品卡片展示价格、商品编号、门店库存、电商仓库存",
    "支持收藏（星标）切换",
    "点击推荐商品向当前会话发送商品卡",
    "返回保留搜索 / 筛选上下文"
   ],
  ...
  "v04-g8": {
   ...
   "overview": {
    ...
    "users": "门店导购",
    "permission": "功能权限：所有页面功能权限；数据权限：当前登录导购的业绩统计数据和订单数据",
    "ports": "中免商户通APP"
   },
   "flow": [
    {
   ...
   "acceptance": [
    "我的业绩 hero 卡本月 / 上月切换",
    "展示已入账（sumCounted）/ 待入账（sumPending）/ 合计（sumCount）",
    "支持搜索、状态筛选、日期范围筛选",
    "订单列表展示成交订单",
    "点击订单底部抽屉展示离岛旅客信息/订单信息/商品/金额明细",
    "仅展示本人业绩与订单"
   ],
   "note": "",
   "hotspots": [
  ...
  "v04-g9": {
   ...
   "api": [],
   "acceptance": [
    "可编辑工号、姓名、EOP组织、品牌（下拉选雅诗兰黛等）、标签、一句话介绍",
    "支持上传头像与形象照",
    "可设置在线状态",
    "保存后生效并回显",
    "仅当前导购可编辑本人资料"
  ...
  "v04-m3": {
   ...
   "overview": {
    ...
    "users": "门店运营",
    "permission": "功能权限：隐藏门店筛选功能；数据权限：只能看到当前门店产生的订单，但不显示同一个订单下其他门店导购的数据",
    "ports": "中免商户通PC端"
   },
   "flow": [
    {
   ...
   "api": [],
   "acceptance": [
    "展示数据卡片（成交客户/订单/金额，含今日+累计）",
    "支持搜索、状态、门店、柜组、导购、日期筛选",
    "支持导出订单",
    "订单表格含离岛信息/提货/柜组列",
    "仅展示当前门店订单，跨门店订单不显示其他导购数据"
   ],
   "note": "",
   "hotspots": [
    {
  ...
  "v04-m4": {
   ...
   "overview": {
    ...
    "users": "门店运营",
    "permission": "功能权限：支持继承客户按钮单独分配权限；数据权限：不涉及数据权限",
    "ports": "中免商户通PC端"
   },
   "flow": [
    {
  ...
  "v04-u3": {
   ...
   "updated": "2026-09-17",
   "kind": "page",
   "url": "v0.4/U3-商品详情页.html",
   "pages": [],
   "images": [],
   "overview": {
    "summary": "用户在 IM 或列表点击商品进入详情页，查看头图轮播、型号、价格与离岛免税信息；在导购模式下展示专属导购与门店库存，可退出切回平台视角，支持 SKU 选择与加购/立即购买。",
    "path": "用户端 → 顾问 IM 会话（U1）/ 奢品顾问列表 → 商品详情（U3）",
    "users": "C端用户",
    "permission": "功能权限：商城用户可见；数据权限：不涉及（公开商品）",
    "ports": "中免海南 APP（安卓、IOS、鸿蒙）、中免海南微信小程序、中免海南支付宝小程序、H5"
   },
   "flow": [],
   "states": [],
   ...
   "acceptance": [
    "头图轮播展示商品图（含 1/8 指示）",
    "型号选择切换商品外观",
    "价格浮层展示售价与预估到手价、离岛免税标识",
    "展示商品编号、发货地、运费信息",
    "导购模式下展示专属导购/门店/库存，可退出导购模式切回平台视角",
    "SKU 选择弹窗可选定规格",
    "底栏含首页/收藏/购物袋/加入购物袋/立即购买",
    "从导购会话进入自动进入导购模式并归因"
   ],
   "note": "",
   "hotspots": []
  ...
  "v04-u17": {
   ...
   "images": [],
   "overview": {
    "summary": "用户完成结算后展示的支付成功页，提示订单已提交并安排发货/离岛提货，提供返回意向单、查看订单、返回顾问对话三个出口。",
    "path": "用户端 → 结算中心（U4）→ 提交支付 → 支付成功（U17）",
    "users": "C端用户",
    "permission": "功能权限：下单用户可见；数据权限：仅本人本次支付结果",
    "ports": "中免海南 APP（安卓、IOS、鸿蒙）、中免海南微信小程序、中免海南支付宝小程序、H5"
   },
   "flow": [],
   "states": [],
